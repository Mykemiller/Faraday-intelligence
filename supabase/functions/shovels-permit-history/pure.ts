...
// RSC-05 permitting-velocity source normalization + the min-n / recency-decay
// aggregation. No network, no Supabase.
...
export function contentBasis(p: RawPermit): string {
  return [p.id, p.status, p.file_date, p.issue_date, p.job_value].join("|");
}
...
export function normalize(p: RawPermit, countyFips: string | null) {
  return {
    ...
    lag_days: lagDays(p.file_date, p.issue_date),
    job_value: p.job_value ?? null,
    building_area_sqft: p.building_area_sqft ?? null,
...
/**
 * RSC-05 velocity for one jurisdiction's permit rows.
 * - plan-review facet only: drops sub-4-day electrical sign-offs so a 2-day hookup
 *   can't masquerade as fast permitting (see permit_type heterogeneity, 2026-07).
 * - MIN_N gate → ABS (null) below threshold; never emit a noisy median.
 * - recency-decayed median (half-life default 2yr) so a 10-yr window leans recent.
 */
...
export function velocity(
  rows: VelRow[],
  opts: { MIN_N?: number; HALF_LIFE_DAYS?: number; ASOF?: string } = {},
): VelResult | null {
  ...
  const weighted = usable.map((r) => ({
    lag: r.lag_days!,
    w: Math.pow(0.5, (asOf - Date.parse(r.issue_date!)) / (HALF_LIFE_DAYS * 86_400_000)),
  }));
...
export function weightedMedian(xs: { lag: number; w: number }[]): number {
  const s = [...xs].sort((a, b) => a.lag - b.lag);
  const half = s.reduce((t, x) => t + x.w, 0) / 2;
  let acc = 0;
  for (const x of s) { acc += x.w; if (acc >= half) return x.lag; }
  return s[s.length - 1].lag;
}
